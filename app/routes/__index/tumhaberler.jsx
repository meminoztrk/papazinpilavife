...
export const loader = async () => {
    const req = await fetch(process.env.REACT_APP_API + "/Blog/GetAll?page=1&items=24", {
        headers: {
...
const tumhaberler = () => {
    const { data, count, API, API_KEY, IMAGES } = useLoaderData();
    ...
    const getBlogs = async (page) => {
        setLoading(true)
        await fetch(API + `/Blog/GetAll?page=${page}&items=24`, {
            method: 'GET',
            headers: {
                'ApiKey': API_KEY,
                'Content-Type': 'application/json'
                                ...
                                <div className="px-6 py-1 before:w-[4px] before:h-full before:bg-red-700 before:block before:rounded-lg before:absolute before:top-0 before:left-0 before:content-['']">
                                    <div>
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="font-semibold text-2xl block">Tüm Haberler</span>
                                        </div>
                                        <span className="text-xs"><Link className="hover:text-red-500" to={"/"}>Anasayfa</Link> {">"} Tüm Haberler</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <Spin spinning={loading}>
                            ...
                            <div className="flex items-center justify-center pb-8">
                                <ReactPaginate
                                    ...
                                    className="flex flex-wrap items-center space-x-2 p-8 pagination"
