...
export const loader = async ({ request }) => {
  ...
  const req = await fetch(process.env.REACT_APP_API + `/Blog/Search?text=${search.get("text")}&page=1`, {
    headers: {
  ...
  // return search.get("text"); // Filters tweets based on the query
};

const arama = () => {
  const { data, API,API_KEY, IMAGES, text } = useLoaderData();
                ...
                <div className="px-6 py-1 before:w-[4px] before:h-full before:bg-red-700 before:block before:rounded-lg before:absolute before:top-0 before:left-0 before:content-['']">
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-semibold text-2xl block">{text} ile ilgili sonuçlar</span>
                    </div>
                    <span className="text-xs"><Link className="hover:text-red-500" to={"/"}>Anasayfa</Link></span>
                  </div>
                </div>
              </div>
            </div>
